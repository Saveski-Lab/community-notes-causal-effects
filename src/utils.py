...
data_root = Path("/data/cn_effect/results_from_public_data_v1/")
...
def clear_and_write(
    ...
    if fp.exists():
        ...
        if (fp.suffix in [".csv", ".parquet"]) and len(prev_entries) > 0:
            # add defaults to prev_entries, if they are not present
            for k, v in defaults.items():
                if k not in prev_entries.columns:
                        prev_entries[k] = v

            same_config_indexer: None | pd.Series = None
            for config_key, config_value in config.items():
                indexer_for_this_key = (
                    (config_key not in prev_entries.columns) or prev_entries[config_key].isna()
        ...
        elif str(fp).endswith(".json.gz") and len(prev_entries) > 0:
            # add defaults to prev_entries, if they are not present
            ...
            for e in prev_entries:
                ...
                for k in config.keys():
                    ...
                    if e_value != config_value and not (
                        (e_value is None) and pd.isna(config_value)
                    ):
                        if logger:
                            logger.info(
...
def read_from_table(fp, config, keep_metadata=False) -> pd.DataFrame | list:
    if fp.suffix == ".parquet":
        # Make sure all columns in config are present in the file
        schema = pq.read_schema(fp)  # metadata-only call
        cols = set(schema.names)
        filters = [
            (k, "==", v)
            for k, v in config.items()
            if k in cols and v is not None and k != "control_list"
        ]
        df = pd.read_parquet(fp, filters=filters)
        # If control_list is specified, filter by it here. It can have a type mismatch if it has been
        # unspecified in all rows, (which gets saved as a bool). If we filter by it in the parquet read, it will fail,
        # as the reader will try to compare a bool to a string.
        if "control_list" in config:
            if "control_list" not in cols:
                df = df.assign(control_list=np.nan)
            if  config["control_list"] is None:
                df = df[df["control_list"].isna() | df["control_list"].isnull()]
            else:
                df = df[df["control_list"] == config["control_list"]].reset_index(drop=True)
        for k, v in config.items():
            if k in cols and v is None:
                df = df[df[k].isna()].reset_index(drop=True)
    elif fp.suffix == ".csv":
        df = pd.read_csv(fp)
        # Filter out any entries with the same configuration as the current run
        same_config_indexer: None | pd.Series = None
    ...
    if not keep_metadata:
        if fp.suffix in [".parquet", ".csv"]:
            df = df.drop(columns=[k for k in config.keys() if k in df.columns])
        elif str(fp).endswith(".json.gz"):
            for k in config.keys():
...
def read_weights(
    ...
    """
    weights_dir = data_root / intermediate_dir / "f_calc_weights"
    ...
    if type(config["matching_metrics"]) is list:
...
def read_control_configs(
    ...
    return {
        tweet_fname.replace(".json.gz", ""): read_from_table(
            data_root / intermediate_dir / "c_find_controls" / tweet_fname,
            metadata_for_step_c,
        )
        for tweet_fname in tqdm(
            os.listdir(data_root / intermediate_dir / "c_find_controls"),
            desc="Reading control configurations",
            smoothing=0,
        )
        if tweet_fname.endswith(".json.gz")
           and len(read_from_table(
            data_root / intermediate_dir / "c_find_controls" / tweet_fname,
            metadata_for_step_c)) > 0
    }


def combine_sequence(
    seq, weights, treatment_column_name="treatment"
):
    """Calculate the synthetic controls based on specified weights."""

    cids = [
        col
        for col in weights.columns[~weights.isna().any()]
    ...
    sums = seq[cids].values * weights[cids].values

    # Filter out columns that have NAs
    sums = sums[:, ~pd.isna(sums).any(axis=0)]
    ...
    # Set treatment
    assert (sums.index == treatment.index).all()
    sums["treatment"] = treatment.values
