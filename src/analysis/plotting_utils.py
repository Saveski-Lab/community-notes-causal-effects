...
from src.utils import read_from_table, data_root
...
def plot_individual_tweet(
    ...
    control_ids = weights[tid].columns[10:]
    ...
    for ax in axs:
        ...
        ######### Set X-axis labels
        ax.set_xlabel("Hours After Note Attached", color=colors["hours"], fontsize=6.4)
...
def plot_overall(
    ...
    if not any([line.get_alpha() == 0.25 for line in ax.lines]):
        ...
        if include_matching_window:
            ax.axvline(0, c=colors["other_2"], alpha=1, zorder=-1000, linestyle=":",
                       linewidth=INDIVIDUAL_LINE_WIDTH)
            ax.axvspan(
                -100,
                0,
    ...
    # Label axes
    ax.set_xlabel("Hours After Note Attached", color=colors["hours"])
...
def plot_bins(
    ...
    bins_are_formatted =  isinstance(bins[0], str) and any(
        ["–" in b or ">" in b or "_" in b for b in bins]
    )
    ...
    for i in range(num_plots):
        ...
        plotting_ax.axvline(
            0, c=colors["other_2"], alpha=1, zorder=-1000, linestyle=":",
            linewidth=INDIVIDUAL_LINE_WIDTH
        )
        plotting_ax.axvspan(
            -100,
        ...
        # Label axes
        plotting_ax.set_xlabel("Hours After Note Attached", color=colors["hours"])
        if include_y_axis_label:
            if y_var == "Increase":
...
def load_tes_for_metrics(metrics, te_config, intermediate_dir):
    ...
    for metric in metrics:
        te_path = (
            data_root / intermediate_dir / "h_treatment_effects" / f"{metric}"
        )
...
def get_metadata():
    tweet_metadata = pd.read_parquet(
        data_root / "tweet_metadata.parquet"
    )
